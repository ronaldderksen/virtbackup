            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _backupDrivers.map((driver) => DropdownMenuItem<String>(value: driver.id, child: Text(driver.label))).toList(),
                ...
                if (_backupDriverId == 'gdrive') ...[
                  ...
                  const SizedBox(height: 12),
                  TextFormField(
                    controller: _gdriveRootPathController,
                    decoration: const InputDecoration(
                  ...
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    initialValue: _gdriveScope,
                    items: const [
                      DropdownMenuItem(value: _BackupServerSetupScreenState._gdriveScopeFile, child: Text('Drive file access (recommended)')),
                      DropdownMenuItem(value: _BackupServerSetupScreenState._gdriveScopeFull, child: Text('Full Drive access')),
                    ],
                    onChanged: (value) {
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Scope',
                      helperText: 'Changing the scope requires reconnecting.',
                  ...
                  Row(
                    children: [
                      FilledButton.icon(
                        onPressed: _isGdriveConnecting ? null : _startGdriveOAuth,
                        icon: const Icon(Icons.link_outlined),
                        label: Text(_isGdriveConnecting ? 'Connecting...' : 'Connect Google Drive'),
                      ),
                      const SizedBox(width: 12),
                      TextButton.icon(onPressed: !_isGdriveConnected() || _isGdriveConnecting ? null : _disconnectGdrive, icon: const Icon(Icons.link_off_outlined), label: const Text('Disconnect')),
                    ],
                  ),
                  const SizedBox(height: 12),
                  if (_isGdriveConnected()) ...[
                    Text('Connected account: ${_agentSettings.gdriveAccountEmail.isEmpty ? 'Unknown' : _agentSettings.gdriveAccountEmail}', style: Theme.of(context).textTheme.bodyMedium),
                    const SizedBox(height: 4),
                    Text('Access token expires: ${_formatGdriveExpiry(_agentSettings.gdriveExpiresAt)}', style: Theme.of(context).textTheme.bodySmall),
                  ] else ...[
